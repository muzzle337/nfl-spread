...
import { withDashboardLineMovementUi } from "./dashboard-line-movement-ui.js";
...
export function withPwaRecovery(html) {
  ...
  const recovery = `
  <style>.pwa-reset-button { width:100%; margin-top:9px; border:1px solid #233041; background:#0e141c; color:#9eb0c2; border-radius:11px; padding:10px 12px; font:750 10px/1.2 system-ui,sans-serif; cursor:pointer; }</style>
  <script>
    (function () {
      ...
      if (justRecovered) {
        ...
        query.delete('recovered'); query.delete('v');
      ...
      fetch('/api/health?client=' + encodeURIComponent(APP_VERSION), { cache:'no-store', headers:{ accept:'application/json' } })
        ...
        .then(function (body) { if (body && body.version && body.version !== APP_VERSION) recover('version-mismatch'); })
        .catch(function () {});
      if ('serviceWorker' in navigator && navigator.serviceWorker.controller && typeof MessageChannel !== 'undefined') {
        setTimeout(function () {
          ...
          var channel = new MessageChannel(); var finished = false;
          var timer = setTimeout(function () { if (!finished) recover('unresponsive-service-worker'); }, 1800);
          channel.port1.onmessage = function (event) {
            finished = true; clearTimeout(timer);
            var version = event.data && event.data.version;
            if (version !== APP_VERSION) recover('service-worker-version-mismatch');
          };
          try { navigator.serviceWorker.controller.postMessage({ type:'GET_VERSION' }, [channel.port2]); }
          catch (_) { clearTimeout(timer); recover('service-worker-check-failed'); }
        }, 600);
      }
    })();
  </script>`;
...
async function withPublicVersion(response) {
  ...
  return new Response(JSON.stringify({ ...body, version: APP_VERSION }), { status: response.status, headers: response.headers });
}
...
async function withPwaShell(response) {
  ...
  const movement = withLineMovementUi(withDashboardLineMovementUi(withPwaRecovery(pwa)));
  return new Response(movement, { status: response.status, headers: response.headers });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname === "/manifest.webmanifest") {
      return textResponse(JSON.stringify(manifestData()), "application/manifest+json; charset=utf-8", { "cache-control": "no-cache" });
    }
    ...
    if (request.method === "GET" && url.pathname === "/icons/icon-192.png") return imageResponse(iconPng(192));
    if (request.method === "GET" && url.pathname === "/icons/icon-512.png") return imageResponse(iconPng(512));
    if (request.method === "GET" && url.pathname === "/icons/apple-touch-icon.png") return imageResponse(iconPng(180));
    ...
    if (request.method === "GET" && url.pathname === "/api/lines/movement/dashboard") {
      if (!env.DB) return jsonResponse({ error: "Database is not bound" }, 503);
      try {
        const target = await resolveDashboardWeek(env.DB);
        if (target.season === null || target.week === null) return jsonResponse({ ok: true, season: target.season, week: target.week, movements: [] });
        const movements = await lineMovementsForWeek(env.DB, target.season, target.week);
        return jsonResponse({ ok: true, season: target.season, week: target.week, movements });
      } catch (error) {
        return jsonResponse({ error: "Dashboard line movement unavailable", message: error.message }, 400);
      }
    }

    const response = await app.fetch(request, env, ctx);
    if (request.method === "GET" && (url.pathname === "/" || url.pathname === "/app")) return withPwaShell(response);
    if (url.pathname === "/api/health") return withPublicVersion(response);
    return response;
  },
