...
test("historical schedule normalization keeps coach/weather/rest/spread fields", () => {
  const g = normalizeHistoricalGame(row());
...
test("coach indicators expose record, spread record and situational samples", () => {
  const rows = [
    row(),
    row({ game_id:"g2", season:2023, gametime:"13:00", home_score:17, away_score:20, result:-3, spread_line:2.5, temp:75, wind:4, home_rest:7, away_rest:7, div_game:0 }),
    row({ game_id:"g3", season:2022, home_score:30, away_score:10, result:20, spread_line:7, temp:95, wind:3, home_rest:8, away_rest:6, div_game:0 })
  ];
